    ...
    return new Promise<void>(async (resolve, reject) => {
      try {
        for (let second = 0; second < durationSeconds; second++) {
          ...
          // Warten wenn pausiert
          while (cleaningProcessRef.current.pause && !cleaningProcessRef.current.cancel) {
            await new Promise((resolve) => setTimeout(resolve, 100))
          }

          // Nochmal prüfen ob abgebrochen wurde nach der Pause
          if (cleaningProcessRef.current.cancel) {
            reject(new Error("Cleaning process cancelled"))
            return
          }
          ...
          setCurrentPumpProgress(Math.round(((second + 1) / durationSeconds) * 100))

          // Kurze Pause zwischen den Sekunden
          await new Promise((resolve) => setTimeout(resolve, 100))
        }
